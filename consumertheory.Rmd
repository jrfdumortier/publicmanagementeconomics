...
2. **Consumer preferences** are expressed using utility functions and indifference curves. Note that the indifference curves represent the contour lines of the utility function. Your preferences are independent of your income.
3. **Consumer choice** results from the combination of the budget constraint and consumer preferences. What are the optimal choices made by the consumer given preferences and constraints? How do those optimal choices change given a change in prices and/or policies?
...
Before moving to consumer preference, consider below a 3-D representation of Mount Saint Helens in Panel (a). In consumer theory, the height of the mountain will represent the utility and the contour lines will represent the indifference curves.
...
- **Diminishing marginal utility**: The more is consumed of a good, the additional utility gained is smaller than for the previous unit.
...
Indifference curves are all combinations of two goods that make the consumer equally well off. Those indifference curves can be represented using an indifference map. A point on a higher indifference curve is preferred to any point on a lower curve. Suppose a utility function of the form
...
curve(indif(x,u3),from=0.5*x3,to=1.5*x3,col="red",add=TRUE)  
    segments(x3,0,x3,y3,lty=2,col="red")
    segments(0,y3,x3,y3,lty=2,col="red")  
rm(a,b,M,px1,px2,px3,py,r,rho,u1,u2,u3,x1,x2,x3,y1,y2,y3,indif,U,xd,yd)
```
...
curve(indif(x,u3),from=0.5*x3,to=1.5*x3,col="red",add=TRUE)  
    segments(x3,0,x3,y3,lty=2,col="red")
    segments(0,y3,x3,y3,lty=2,col="red") 
rm(bc,a,b,M,M1,M2,px,px1,px2,py,T1,t1,T2,t2,T3,t3,u1,u2,u3,x1,x2,x3,y1,y2,y3,indif,
   U,xd,yd,pmd,i,Q,exmd,funmd,funQ1,funQ2,funQ3)
```

|           | Budget line                           | Choice               
|-----------|---------------------------------------|-------------------------
Original    | $P_x \cdot Q_x + P_y \cdot Q_y = M$   | ($x_1^\star,y_1^\star$)   
Per-unit    | $(P_x + t) \cdot Q_x + P_y Q_y = M$   | ($x_2^\star,y_2^\star$)   
...
**MPG Illusion**: As a second example, suppose that three people drive 10,000 miles per year \citep{Larrick:2008}. The fuel economy of the current cars are 10 MPG for person A, 16.5 MPG for person B, and 33 MPG for person C. They all trade-in their current cars for new cars that get 11, 20, and 50 MPG for A, B, and C, respectively. Who saves the most gas after the trade-in assuming that their driving habits do not change? See Figure \ref{fig:CON_mpg} for a graphical representation of the relationship between MPG and gasoline consumption. Table \ref{table:mpgillusion}) compares using MPG to using gallons per mile. \\
...
gallons        = function(x){milesdriven/x}
     curve(gallons,from=0,to=50,ylab="Gallons per 10,000 Miles",xlab="Miles per Gallon",
           ylim=c(0,2500),yaxs="i",xaxs="i")  
     graphics.off()
...
3. *Revealed Preferences*: To better understand preferences, researchers often present respondents with different price and income situations and ask them to choose between various consumption bundles (see \citet{Grebitus:2016} for an example). Consider the following two situations:
...
rm(a,M,px,py,T,t,t_rev1,u0,u1,x0,x1,y0,y1,bc,indif,U,xd,yd)
```
