...
ggplot(data=gasoline,aes(x=Year,y=value,group=Legend))+geom_line(aes(linetype=Legend),size=0.5)+
    theme_bw()+theme(legend.position="bottom",axis.title.x=element_blank())+ylim(0,5)+ylab("Price in $ per Gallon")
rm(gasoline)
```
...
ggplot(data=infueltax,aes(x=Year,y=value/1000,group=Legend))+geom_line(aes(linetype=Legend),size=0.5)+theme_bw()+
    theme(legend.position="bottom",axis.title.x=element_blank())+ylab("Revenue in Million $")+ylim(0,1500)
rm(infueltax)
```
